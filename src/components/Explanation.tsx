        ...
        <div>
            ...
            <h2 style={{textAlign: 'center', margin: '30px 0'}}>编码器 (Encoder)</h2>
             <MathBlock id="token_embed" title="编码器第0步：分词与词嵌入" highlight={highlight}>
                ...
                <ol>
                    ...
                    <li><b>ID映射 (ID Mapping):</b> 使用一个预先构建好的词汇表（Vocabulary），将每个词元映射到一个唯一的整数ID。例如 "I" → 3, "am" → 4。</li>
            ...
            <MathBlock id="mha" title="编码器子层1：多头自注意力" highlight={highlight}>
                ...
                <div className="formula-display">
                    ...
                    <InteractiveSymbolicMatrix name={LNe.encoder_input} rows={dims.seq_len} cols={dims.d_model} highlight={highlight} onSymbolClick={onSymbolClick} truncate={false} />
                </div>
                <h5>计算 Q, K, V (Query, Key, Value)</h5>
            ...
            <MathBlock id="add_norm_1" title="组件：残差连接与层归一化 (1)" highlight={highlight}>
                ...
                <BlockMath math={`Z' = \\text{LayerNorm}(Z + \\text{Sublayer}(Z))`} />
                 <div className={`formula-display ${shouldBreakAddNorm ? 'vertical' : ''}`}>
                    <div className="matrix-scroll-wrapper"><InteractiveSymbolicMatrix name={LNe.encoder_input} rows={dims.seq_len} cols={dims.d_model} highlight={highlight} onSymbolClick={onSymbolClick} /></div>
                    <BlockMath math="+" />
            ...
            <MathBlock id="ffn" title="编码器子层2：位置前馈网络" highlight={highlight}>
                ...
                <div className="formula-display vertical">
                    <div className="viz-formula-row"><InteractiveSymbolicMatrix name={LNe.add_norm_1_output} rows={dims.seq_len} cols={dims.d_model} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                    <BlockMath math="\times"/>
                    <div className="viz-formula-row"><InteractiveSymbolicMatrix name={LNe.W1} rows={dims.d_model} cols={dims.d_ff} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                    <BlockMath math="+"/>
                    <div className="viz-formula-row"><InteractiveSymbolicVector name={LNe.b1} data={Array(dims.d_ff).fill(0)} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                    <BlockMath math="\xrightarrow{ReLU}"/>
                    <div className="viz-formula-row"><InteractiveSymbolicMatrix name={LNe.Activated} rows={dims.seq_len} cols={dims.d_ff} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                </div>
            </MathBlock>
            ...
            <MathBlock id="ffn_dec" title="解码器子层3：前馈网络" highlight={highlight}>
                ...
                <BlockMath math={`F = \\text{ReLU}(Y'' W_1 + b_1) W_2 + b_2`} />
                 <div className="formula-display vertical">
                     <div className="viz-formula-row"><InteractiveSymbolicMatrix name={LNd.add_norm_2_output} rows={dims.seq_len} cols={dims.d_model} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                    <BlockMath math="\times"/>
                    ...
                    <BlockMath math="\xrightarrow{ReLU}"/>
                    <div className="viz-formula-row"><InteractiveSymbolicMatrix name={LNd.Activated} rows={dims.seq_len} cols={dims.d_ff} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                </div>
            </MathBlock>
             ...
             <MathBlock id="final_output" title="最终输出层：线性层与Softmax" highlight={highlight}>
                ...
                <ol>
                 ...
                 <div className="formula-display">
                    <div className="matrix-scroll-wrapper"><InteractiveSymbolicMatrix name={LNd.add_norm_3_output} rows={dims.seq_len} cols={dims.d_model} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                    <BlockMath math="\times" />
                    <div className="matrix-scroll-wrapper"><InteractiveSymbolicMatrix name={MATRIX_NAMES.finalLinear} rows={dims.d_model} cols={16} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                    <BlockMath math="\xrightarrow{\text{Softmax}}" />
                    <div className="matrix-scroll-wrapper"><InteractiveSymbolicMatrix name={MATRIX_NAMES.outputProbabilities} rows={dims.seq_len} cols={16} highlight={highlight} onSymbolClick={onSymbolClick}/></div>
                </div>
